...
- Raises when a value is missing for a directive, or is not of the kind that directive expects.
- <Since v="5.3" /> Raises when a float with a fractional part reaches an integer directive such as `%d`.
...
- `%s` — a string. A value that is not already one is converted first, by the rules `tostring()` uses.
...
- `%a`, `%A` — a number, as a hexadecimal float, with lower- or upper-case letters.
...
Flags and sizing follow C, from a set Lua fixes: the flags are `-`, `+`, `#`, `0` and a
space, and a width and a precision run to two digits at most. `%5d` pads to five
columns, `%-5d` pads on the right instead, and `%.3f` keeps three decimal places. Those
flags and the conversions listed above are the whole of what a directive may hold.
...
trip. It takes no flags, width or precision of its own.
</Note>
